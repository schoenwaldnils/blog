...
import { tests, CIRCLE_SHA1, GOOGLE_PAGESPEED_API_KEY } from './psiVars'
...
async function runPagespeed({ strategy, categories }) {
  ...
  const paramString = qs
    .stringify(params)
    .replace('category=REPLACEME', categoriesEdited.join('&'))

  return fetch(
    `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?${paramString}`,
  )
    .then(res => {
...
async function postgithubStatus({
  ...
  const result = await gitStatus({
    ...
    description: `${state.toUpperCase()}: Score: ${100 * score}; min: ${100 *
      minExpectedScore}`,
...
function runTests() {
  ...
  Promise.all(
    tests.map(async ({ strategy, categories, minExpectedScore }) => {
