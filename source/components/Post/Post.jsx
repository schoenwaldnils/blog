...
const Post = props => {
  ...
  const classNames = cn('Post', { [`Post--${type}`]: type })
    ...
    <article className={classNames}>
        ...
        <div className="Post-wrapImage">
          ...
          {title && (
            <div className="Post-meta">
              <h1 className="Post-title Post-title--inImage u-boxPadding u-maxWidth">
                {title}
              </h1>
            </div>
          )}
      ...
      {content && type !== 'list' && (
        <div className="Post-content u-boxPadding u-maxWidth">
          {!image && (
            <h1 className="Post-title Post-title--inContent">{title}</h1>
          )}
          <div className="u-richText">{marked(content)}</div>
        </div>
      )}
